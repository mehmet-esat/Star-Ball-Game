// Eski kodundaki let players = [{...}] ve let localPlayerId = 1 kısımlarını silip ŞUNU YAPIŞTIR:

const socket = io();
let players = []; // Artık bu liste sunucudan dolacak
let localPlayerId = null; 

socket.on('connect', () => {
    console.log("Sunucuya başarıyla bağlandım! Benim ID numaram:", socket.id);
    localPlayerId = socket.id; // Kendi ID numaramızı Telsizden gelen ID yapıyoruz
});

// Sunucudan Telsizle "Liste güncellendi" mesajı geldiğinde lobiyi yenile
...
function showNicknameModal() {
    ...
    // YENİ EKLENEN KISIM: İsmi kendi bilgisayarımızda değil, Telsizle sunucuya bildiriyoruz
...
function joinTeam(teamName) {
    ...
    // YENİ EKLENEN KISIM: Hangi takıma tıkladığımızı sunucuya söylüyoruz
...
function update() {
    ...
    if (isPlaying) {
        ...
        player.update(); resolveCollision(player, ball); posts.forEach(post => { resolveCollision(player, post); });
...
function draw() {
    ...
    if (locPlayer.team !== 'spectator') {
        ...
        let avatarText = locPlayer.avatar || '7'; ctx.fillText(avatarText, player.x, player.y + 2);
        ...
        let dynamicLabel = `${locPlayer.name} ${locPlayer.goals > 0 ? '(' + locPlayer.goals + ')' : ''}`;
