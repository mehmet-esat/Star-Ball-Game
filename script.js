...
const ADMIN_PASSWORD = 'esatmete1925';
...
function handleNicknameSubmit() {
    ...
    if (isAdmin) {
        addChatMessage('Sistem', `Admin olarak katıldın: ${nick}`, '#f1c40f');
    } else {
        addChatMessage('Sistem', `Odaya katıldın: ${nick}`, '#8b949e');
    }
}
...
function copyRoomLink() {
    ...
    addChatMessage("Sistem", "Oda linki kopyalandı!", "lightgreen");
}
...
function joinTeam(teamName) {
    ...
    if (isGameActive && p.team === 'spectator' && teamName !== 'spectator') {
        const spawnDistX = FIELD_BOUNDS.left + ((L_WIDTH / 2) - FIELD_BOUNDS.left) * 0.5;
        player.x = teamName === 'red' ? (L_WIDTH / 2) - (L_WIDTH / 2 - spawnDistX) : (L_WIDTH / 2) + (L_WIDTH / 2 - spawnDistX);
        player.y = L_HEIGHT / 2; player.vx = 0; player.vy = 0;
    }
    if(teamName === 'red') player.color = '#f2504b';
    else if(teamName === 'blue') player.color = '#5784f2';
}
...
function toggleGameState() {
    ...
    if (isGameActive) {
        ...
        ball.radius = 10; ball.color = '#ffed4f';
        resetPositions();
        addChatMessage("Sistem", "Oyun başladı!", "lightgreen");
        isGameActive = true;
    }
}
...
setupMenu.addEventListener('click', (e) => {
    if (e.target === setupMenu) { setupMenu.classList.remove('visible'); if (isGameActive) isPaused = false; }
});
...
function kickSelectedPlayer() {
    ...
    if (p) {
        p.isKicked = true; p.team = 'spectator';
        updateLobbyUI();
        addChatMessage("Sistem", `${p.name} odadan atıldı (Kick).`, "red");
    }
}
...
window.addEventListener('keydown', (e) => {
    ...
    if((k === ' ' || k === 'x' || k === '0' || e.code === 'Numpad0') && !keys.space) {
        keys.space = true; isKicking = true;
    }
    if((k === 'f') && !keys.f) {
        keys.f = true; isPassing = true;
    }
});
...
function restrictKickoff(entity, teamName) {
    ...
    if (isDefending) {
        if (teamName === 'blue') {
            if (entity.x - entity.radius < midX) { entity.x = midX + entity.radius; if (entity.vx < 0) entity.vx = 0; }
...
const player = new CircleEntity(0, 0, 22, 2, '#f2504b', false);
const ball = new CircleEntity(0, 0, 10, 1, '#ffed4f', true); 
let posts = [];
    ...
    if (distance < minDist && distance > 0) {
        if (kickoffState.active) {
            if ((c1 === player && c2 === ball) || (c1 === ball && c2 === player)) {
                ...
                if (locPlayer && locPlayer.team === kickoffState.concedingTeam) { 
        ...
        c1.x -= nx * overlap * (c1.invMass / totalInvMass);
        c1.y -= ny * overlap * (c1.invMass / totalInvMass);
        c2.x += nx * overlap * (c2.invMass / totalInvMass);
        c2.y += ny * overlap * (c2.invMass / totalInvMass);

        const rvx = c2.vx - c1.vx; const rvy = c2.vy - c1.vy;
...
function checkWallCollisions(entity) {
    ...
    if (entity.y - entity.radius < FIELD_BOUNDS.top) { 
        entity.y = FIELD_BOUNDS.top + entity.radius; entity.vy *= -0.50; 
    } else if (entity.y + entity.radius > FIELD_BOUNDS.bottom) { 
        entity.y = FIELD_BOUNDS.bottom - entity.radius; entity.vy *= -0.50; 
    }

    if (entity.x - entity.radius < FIELD_BOUNDS.left) {
        if (!isInsideGoalY) { 
            entity.x = FIELD_BOUNDS.left + entity.radius; entity.vx *= -0.50; 
        } else if (entity.x - entity.radius < GOAL_BACK_BOUNDS.left) { 
            entity.x = GOAL_BACK_BOUNDS.left + entity.radius; entity.vx *= -0.50; 
        }
    }
    
    if (entity.x + entity.radius > FIELD_BOUNDS.right) {
        if (!isInsideGoalY) { 
            entity.x = FIELD_BOUNDS.right - entity.radius; entity.vx *= -0.50; 
        } else if (entity.x + entity.radius > GOAL_BACK_BOUNDS.right) { 
            entity.x = GOAL_BACK_BOUNDS.right - entity.radius; entity.vx *= -0.50; 
        }
    }
...
function drawCustomMap() {
    const midX = L_WIDTH / 2; const midY = L_HEIGHT / 2;
    if (customMapData.bg && customMapData.bg.color) {
        ctx.fillStyle = '#' + customMapData.bg.color; ctx.fillRect(0, 0, L_WIDTH, L_HEIGHT);
    } else {
        ctx.fillStyle = '#2b2c3a'; ctx.fillRect(0, 0, L_WIDTH, L_HEIGHT);
    }
    ...
    if (customMapData.segments && customMapData.vertexes) {
        customMapData.segments.forEach(seg => {
            ...
            ctx.strokeStyle = seg.color ? '#' + seg.color : '#ffffff';
            ctx.lineWidth = 3; ctx.beginPath();
            let p0x = midX + (v0.x || 0); let p0y = midY + (v0.y || 0);
            let p1x = midX + (v1.x || 0); let p1y = midY + (v1.y || 0);

            if (!seg.curve || seg.curve === 0) {
                ctx.moveTo(p0x, p0y); ctx.lineTo(p1x, p1y);
            } else {
                let curveRad = seg.curve * Math.PI / 180;
                let dx = p1x - p0x; let dy = p1y - p0y;
                let d = Math.hypot(dx, dy);
                if (d > 0) {
                    let r = Math.abs((d / 2) / Math.sin(curveRad / 2));
                    let h = (d / 2) / Math.tan(curveRad / 2);
...
function update() {
    ...
    let locPlayer = players.find(p => p.id === localPlayerId);
    let isPlaying = (locPlayer && (locPlayer.team === 'red' || locPlayer.team === 'blue'));
    ...
    if (isPlaying) {
        ...
        restrictKickoff(player, locPlayer.team); 
    }

    ball.update();
    posts.forEach(post => { resolveCollision(ball, post); });
    checkWallCollisions(ball); checkGoal();
}
...
function checkGoal() {
    ...
    if (isInsideGoalY) {
        if (ball.x + ball.radius < FIELD_BOUNDS.left) {
            isGoalScored = true; score.blue++; kickoffState.concedingTeam = 'red'; playGoalSound(); processGoalEvent("Blue Scores!", "#5784f2");
        }
        else if (ball.x - ball.radius > FIELD_BOUNDS.right) {
            isGoalScored = true; score.red++; kickoffState.concedingTeam = 'blue'; playGoalSound(); processGoalEvent("Red Scores!", "#f2504b");
        }
    }
}
...
function resetPositions() {
    ...
    let locPlayer = players.find(p => p.id === localPlayerId);
    if(locPlayer && locPlayer.team !== 'spectator') {
        const spawnDistX = FIELD_BOUNDS.left + ((L_WIDTH / 2) - FIELD_BOUNDS.left) * 0.5;
        if (locPlayer.team === 'red') { player.x = (L_WIDTH / 2) - (L_WIDTH / 2 - spawnDistX); } 
        else { player.x = (L_WIDTH / 2) + (L_WIDTH / 2 - spawnDistX); }
        player.y = L_HEIGHT / 2; player.vx = 0; player.vy = 0;
    }
    updateHeaderUI();
}
...
setInterval(() => {
    if (!isGameActive || isPaused || kickoffState.active) return; 
    gameTime++; updateHeaderUI(); checkTimeEnd(); 
}, 1000);
...
function draw() {
    ...
    if (locPlayer) {
        if (zoomLevel > 1) { 
            ...
            if (locPlayer.team === 'spectator') { ctx.translate(-L_WIDTH/2, -L_HEIGHT/2); } 
    ...
    if (locPlayer && locPlayer.team !== 'spectator') {
        if ((keys.space || keys.f) && isGameActive && !isChatFocused && !isPaused) {
        ...
        let avatarText = '7'; ctx.fillText(avatarText, player.x, player.y + 2);

        const isOutOfPitch = player.x < FIELD_BOUNDS.left || player.x > FIELD_BOUNDS.right || player.y < FIELD_BOUNDS.top || player.y > FIELD_BOUNDS.bottom;
        ctx.fillStyle = locPlayer.team === 'red' ? '#f2504b' : '#5784f2'; ctx.font = 'bold 16px Arial'; ctx.textAlign = 'center'; ctx.textBaseline = 'top';

        let dynamicLabel = `${locPlayer.name}`;
        if (isOutOfPitch) { ctx.fillStyle = 'rgba(255,255,255,0.7)'; dynamicLabel += " (Dışarıda)"; }
        ctx.fillText(dynamicLabel, player.x, player.y + player.radius + 15);
    }
